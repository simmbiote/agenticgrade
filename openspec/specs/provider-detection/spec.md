...
Detect which agentic coding provider(s) (OpenSpec, Claude, a generic AGENTS.md-based "universal" setup, or none) are present in a scanned repository, and expose that detected provider set to the metrics catalog and scoring engine so provider-scoped metrics can be selected and scored correctly.
